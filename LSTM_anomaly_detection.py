...
x_whole, y_whole = create_dataset(x_whole, look_back)
...
plt.grid(True)
plt.show()
fig.savefig(os.path.join('fig', 'loss.png'))

fig = plt.figure(figsize=(14,8))
plt.title('acc')
plt.plot(np.arange(len(acc)), acc, label="training loss")
plt.legend()
plt.xlim(0,epochs)
plt.ylim(0,1.0)
plt.xlabel('epochs')
...
plt.grid(True)
plt.show()
fig.savefig(os.path.join('fig', 'acc.png'))
...
whole_score = math.sqrt(mean_squared_error(y_whole[0], y_pred[:,0]))
print('Whole RMSE: %.2f' % (whole_score))
...
plt.grid(True)
plt.show()
fig.savefig(os.path.join('fig', 'predict.png'))


#####################
# 異常検知
#####################
y_whole = np.reshape(y_whole, (y_whole.shape[1], y_whole.shape[0]))

# 実測値と予測値の差
diff = y_pred_plot - scaler.inverse_transform(dataset)
...
plt.grid(True)
plt.show()
fig.savefig(os.path.join('fig', 'difference.png'))
